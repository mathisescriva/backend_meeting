...
logger = logging.getLogger("sqlite-assemblyai")
logger.setLevel(logging.INFO)
handler = logging.StreamHandler()
handler.setFormatter(logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s'))
logger.addHandler(handler)

router = APIRouter(prefix="/sqlite", tags=["SQLite AssemblyAI"])

# Chemin vers la base de donnu00e9es SQLite
SQLITE_DB_PATH = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "app.db")
...
async def sqlite_upload_meeting(
    ...
    Cette route utilise SQLite pour stocker les donnu00e9es, mu00eame lorsque l'application est configuru00e9e pour PostgreSQL.
...
@router.get("/meetings", response_model=list, status_code=200)
async def get_sqlite_meetings(
    current_user: dict = Depends(get_current_user)
):
    """
    Ru00e9cupu00e8re toutes les ru00e9unions de l'utilisateur depuis SQLite.
    """
    # Vu00e9rification explicite de l'authentification
    if not current_user or "id" not in current_user:
        raise HTTPException(
            status_code=401,
            detail={
    ...
    try:
        # Utiliser SQLite pour ru00e9cupu00e9rer les donnu00e9es
        conn = get_sqlite_connection()
        cursor = conn.cursor()
        
        # Ru00e9cupu00e9rer toutes les ru00e9unions de l'utilisateur
        cursor.execute(
        ...
        # Convertir les donnu00e9es en liste de dictionnaires
        result = [dict(meeting) for meeting in meetings]
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Une erreur s'est produite lors de la ru00e9cupu00e9ration des ru00e9unions: {str(e)}"
        )

@router.get("/meetings/{meeting_id}", response_model=dict, status_code=200)
async def get_sqlite_meeting(
    meeting_id: str,
    current_user: dict = Depends(get_current_user)
):
    """
    Ru00e9cupu00e8re une ru00e9union spu00e9cifique depuis SQLite.
    """
    # Vu00e9rification explicite de l'authentification
    if not current_user or "id" not in current_user:
        raise HTTPException(
    ...
    try:
        ...
        # Ru00e9cupu00e9rer la ru00e9union spu00e9cifique
        cursor.execute(
        ...
        if not meeting:
            raise HTTPException(
                status_code=404,
                detail="Ru00e9union non trouvu00e9e"
            )
        ...
        logger.error(f"Erreur lors de la ru00e9cupu00e9ration de la ru00e9union: {str(e)}")
        raise HTTPException(
            status_code=500,
            detail=f"Une erreur s'est produite lors de la ru00e9cupu00e9ration de la ru00e9union: {str(e)}"
        )
