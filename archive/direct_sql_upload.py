...
logger = logging.getLogger("direct-sql-upload")
logger.setLevel(logging.INFO)
handler = logging.StreamHandler()
handler.setFormatter(logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s'))
logger.addHandler(handler)

router = APIRouter(prefix="/direct-sql", tags=["Direct SQL Upload"])
...
async def direct_sql_upload_meeting(
    ...
    Cette route utilise des requu00eates SQL directes avec PostgreSQL.
    ...
    try:
        ...
        cursor.execute("SELECT COUNT(*) FROM users WHERE id = %s", (user_id_int,))
