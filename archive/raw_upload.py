...
logger = logging.getLogger("raw-upload")
logger.setLevel(logging.INFO)
handler = logging.StreamHandler()
handler.setFormatter(logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s'))
logger.addHandler(handler)

router = APIRouter(prefix="/raw", tags=["Raw Upload"])
...
async def raw_upload_meeting(
    ...
    Cette route utilise une approche brute avec SQL litu00e9ral sans paramu00e8tres.
    ...
    try:
        ...
        # Insu00e9rer la ru00e9union dans PostgreSQL avec une requu00eate brute
        logger.info(f"Insertion de la ru00e9union {meeting_id} pour l'utilisateur {user_id_int}")
        
        # Nettoyer les valeurs pour u00e9viter les injections SQL
        safe_title = title.replace("'", "''") if title else "Sans titre"
        safe_file_url = file_url.replace("'", "''")
        
        # Utiliser une requu00eate SQL brute sans paramu00e8tres
        query = f"""
        INSERT INTO meetings (id, user_id, title, file_url, transcript_status, created_at)
        VALUES ('{meeting_id}', 1, '{safe_title}', '{safe_file_url}', 'processing', '{created_at}')
        RETURNING *
        """
        
        logger.info(f"Exu00e9cution de la requu00eate brute: {query}")
