...
logger = logging.getLogger("ensure-upload")
logger.setLevel(logging.INFO)
handler = logging.StreamHandler()
handler.setFormatter(logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s'))
logger.addHandler(handler)

router = APIRouter(prefix="/ensure", tags=["Ensure Upload"])
...
# Fonction pour s'assurer que l'utilisateur existe
def ensure_user_exists(user_id, email="admin@example.com", full_name="Admin User", password_hash="$2b$12$Ta4F.ii95grBIy9Z5T7pVO.Ly6Olty2lr.yHSKnj9RWY6X0CdFM26"):
    ...
    try:
        ...
        if not user_exists:
            # Cru00e9er l'utilisateur s'il n'existe pas
            logger.info(f"L'utilisateur avec l'ID {user_id} n'existe pas, cru00e9ation...")
            cursor.execute(
                """
                INSERT INTO users (id, email, hashed_password, full_name, created_at)
                VALUES (%s, %s, %s, %s, %s)
                """,
                (user_id, email, password_hash, full_name, datetime.utcnow())
            )
            conn.commit()
            logger.info(f"Utilisateur avec l'ID {user_id} cru00e9u00e9 avec succu00e8s")
            return True
        else:
            logger.info(f"L'utilisateur avec l'ID {user_id} existe du00e9ju00e0")
            return True
    except Exception as e:
        logger.error(f"Erreur lors de la vu00e9rification/cru00e9ation de l'utilisateur: {str(e)}")
        conn.rollback()
        return False
    finally:
        cursor.close()
...
async def ensure_upload_meeting(
    ...
    Cette route s'assure que l'utilisateur existe avant d'insu00e9rer la ru00e9union.
    ...
    if not ensure_user_exists(user_id_int):
        raise HTTPException(
            status_code=500,
            detail="Impossible de s'assurer que l'utilisateur existe"
        )
    ...
    try:
        ...
        # Insu00e9rer la ru00e9union dans PostgreSQL
