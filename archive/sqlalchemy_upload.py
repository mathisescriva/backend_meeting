...
from ..db.sqlalchemy_database_fixed import SessionLocal, Meeting

# Logger pour le du00e9bogage
logger = logging.getLogger("sqlalchemy-upload")
logger.setLevel(logging.INFO)
handler = logging.StreamHandler()
handler.setFormatter(logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s'))
logger.addHandler(handler)

router = APIRouter(prefix="/sqlalchemy", tags=["SQLAlchemy Upload"])
...
async def sqlalchemy_upload_meeting(
    ...
    Cette route utilise SQLAlchemy avec la base de donnu00e9es PostgreSQL.
    ...
    try:
        ...
        # Cru00e9er un nouvel objet Meeting avec uniquement les colonnes essentielles
        new_meeting = Meeting(
            id=meeting_id,
            user_id=user_id,
            title=title,
            ...
            # Ne pas inclure les colonnes transcript_* pour u00e9viter les erreurs
            created_at=created_at
        )
        
        # Du00e9sactiver temporairement les attributs transcript_* pour u00e9viter les erreurs
        new_meeting.transcript_content = None
        new_meeting.transcript_summary = None
        new_meeting.transcript_key_points = None
        new_meeting.transcript_action_items = None
