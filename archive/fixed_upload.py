...
logger = logging.getLogger("fixed-upload")
logger.setLevel(logging.INFO)
handler = logging.StreamHandler()
handler.setFormatter(logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s'))
logger.addHandler(handler)

router = APIRouter(prefix="/fixed", tags=["Fixed Upload"])
...
async def fixed_upload_meeting(
    ...
    Cette route utilise des requu00eates SQL directes avec PostgreSQL, corrigu00e9es pour fonctionner correctement.
    ...
    try:
        ...
        # Utiliser une requu00eate directe avec l'ID en dur (fonctionne d'apru00e8s nos tests)
        cursor.execute(f"SELECT COUNT(*) FROM users WHERE id = {user_id_int}")
        ...
        # Insu00e9rer la ru00e9union dans PostgreSQL avec l'ID utilisateur en dur
