...
LOG_FILE = f"test_logs/summary_agent_test_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
def generate_summary(token, meeting_id):
    ...
    log(f"\n===== Gu00c9Nu00c9RATION DU COMPTE RENDU AVEC L'AGENT MISTRAL (Meeting ID: {meeting_id}) =====")
...
def main():
    ...
    log(f"\n{'='*50}")
    log(f"Du00c9BUT DU TEST DE Gu00c9Nu00c9RATION DE COMPTE RENDU AVEC L'AGENT MISTRAL - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    log(f"{'='*50}\n")
    ...
    # Gu00e9nu00e9rer un nouveau compte rendu avec l'agent Mistral
    ...
    if status == "completed" and summary_data and summary_data.get("summary_text"):
        log("\n===== COMPTE RENDU Gu00c9Nu00c9Ru00c9 PAR L'AGENT MISTRAL =====")
